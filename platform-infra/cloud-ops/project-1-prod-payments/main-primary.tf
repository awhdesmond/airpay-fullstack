...
module "subnet_default" {
  ...
  subnet_primary_ip_cidr = "10.1.0.0/16"
}
...
module "subnet_gke" {
  ...
  subnet_primary_ip_cidr = "10.20.0.0/16"
  subnet_secondary_ip_cidrs = [
    { name = local.gke_cluster_pod_range_name, ip_cidr_range = "10.21.0.0/16" },
    { name = local.gke_cluster_svc_range_name, ip_cidr_range = "10.22.0.0/16" }
  ]
}
